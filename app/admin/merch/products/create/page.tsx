...
const PRODUCT_TYPE_SUGGESTIONS = ['T-SHIRT', 'HOODIE', 'JACKET', 'PANTS', 'HAT']
...
export default function CreateProductPage() {
    ...
    const [sizeInputs, setSizeInputs] = useState<{ [rowId: number]: string }>(
        {}
    )
    ...
    useEffect(() => {
        if (debouncedProductType.trim() === '') {
            setFilteredProductTypes([])
            return
        }

        const matches = PRODUCT_TYPE_SUGGESTIONS.filter((type) =>
            type.toLowerCase().includes(debouncedProductType.toLowerCase())
        )

        setFilteredProductTypes(matches)
    }, [debouncedProductType])

    useEffect(() => {
        const timers: { [id: number]: NodeJS.Timeout } = {}

        Object.entries(sizeInputs).forEach(([rowIdStr, value]) => {
            const rowId = parseInt(rowIdStr, 10)
            if (timers[rowId]) clearTimeout(timers[rowId])

            timers[rowId] = setTimeout(() => {
                if (value.trim() === '') {
                    setFilteredSizes((prev) => ({ ...prev, [rowId]: [] }))
                    return
                }

                const matches = SIZE_SUGGESTIONS.filter((size) =>
                    size.toLowerCase().includes(value.toLowerCase())
                )

                setFilteredSizes((prev) => ({ ...prev, [rowId]: matches }))
            }, 300)
        })
                                ...
                                {variant.sizeRows.map((row, index) => (
                                    ...
                                    >
                                        <div className="relative">
                                            ...
                                            {filteredSizes[row.id]?.length >
                                                0 && (
                                                <ul className="absolute z-10 bg-white border text-black border-gray-300 w-full mt-1 rounded shadow-md max-h-40 overflow-y-auto">
                                                    {filteredSizes[row.id].map(
                                                        (size, idx) => (
                                                            <li
                                                                key={idx}
                                                                onClick={() => {
