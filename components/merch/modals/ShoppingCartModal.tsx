        ...
        setCartItems((prevItems) => {
            const updatedItems = prevItems.map((item, i) => {
                if (i === index) {
                    ...
                    setTotalPrice((prevTotal) => isCheckedOut ? prevTotal + priceChange : prevTotal - priceChange);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="flex flex-col bg-main-dark text-white p-4 xs:p-6 rounded-2xl max-w-3xl w-full relative border-csg-blue-400 border-2 right-0.5 overflow-hidden">
                ...
                <div className="hidden md:flex w-full h-fit flex-row justify- gap-4 rounded-2xl text-center text-[13px] sm:text-xs md:text-sm bg-black text-[#434365] font-medium font-vietnam tracking-wider">
                    <div className="w-2/5 ml-16 truncate whitespace-nowrap">PRODUCT DETAILS</div>
                ...
                <div className="h-96 overflow-y-auto hidden-scrollbar mb-3">
                    {cartItems.map((merch, index) => (
                        <ShoppingCartItem
                            ...
                            onCheckoutChange={(itemIndex, newCheckedState) => handleCheckoutChange(itemIndex, newCheckedState) }
                        />
                    ))}
                </div>
                <div className="absolute flex flex-row h-fit w-full bg-black bottom-0 left-0 right-0 p-0 m-0 text-center justify-center align-center z-30 text-xs md:text-sm">
                    <div className={`w-[16px] ps:w-[24px] md:w-[35px] aspect-square rounded-full border-2 md:border-4 border-[#171745] flex shrink-0 cursor-pointer items-center justify-center ${allChecked ? 'bg-[#2929b7]' : 'bg-[#2929b7]/0'
                        }`} />
                    <p className="basis-1/4 self-center">ALL</p>
                    <div className="basis-1/4 self-center">
                        <span className="text-[#242460] font-bold">TOTAL</span> PHP { totalPrice }
                    </div>
