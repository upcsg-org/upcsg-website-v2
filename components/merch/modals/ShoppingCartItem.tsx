...
const ShoppingCartItem = (props: ShoppingCartItem) => {
    ...
    const { id, merch, quantity, size, color, maxQuantity, isCheckedOut, onCheckoutChange } = props
    ...
    return (
        ...
        <div className="min-w-4 flex flex-row items-center gap-1 ps:gap-2 md:gap-4 overflow-hidden">
            {/*button*/}
            <div onClick={handleToggle} className={`w-[16px] ps:w-[24px] md:w-[35px] aspect-square rounded-full border-2 md:border-4 border-[#171745] flex shrink-0 cursor-pointer items-center justify-center ${IsCheckedOut ? 'bg-[#2929b7]' : 'bg-[#2929b7]/0'
                }`} />
            {/*outline*/}
            <div className="min-w-4 flex flex-row gap-4 md:gap-8 border rounded-lg p-3 items-left w-full mb-2 mt-2">
                ...
                <div className="flex w-50 grow md:w-1/2 flex-row gap-2 md:gap-10">
                    {/*size */}
                    <select value={size.text} onChange={(e) => {
                        const selectedSize = sizes.find(s => s.text === e.target.value)
                        if (selectedSize) {
